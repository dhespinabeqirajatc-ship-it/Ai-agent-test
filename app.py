...
def _query_flag(name):
    """Return True for common truthy URL query-parameter values."""
    try:
...
PRESENTATION_QUERY_MODE = _query_flag("presentation")
EMBED_QUERY_MODE = _query_flag("embed")


def _query_value(name, default=None):
    """Return one URL query-parameter value without assuming Streamlit list semantics."""
    try:
...
PRESENTATION_REQUEST_QUERY = _query_value("dashboard_request", "")
...
def render_ai_evidence_indicator(summary, purpose="analysis", forecast_result=None, comparison_result=None, key=None, compact=False):
    """Confidence/evidence-rating UI was intentionally removed.

    Grounding and deterministic calculations remain active; this compatibility
    shim keeps existing call sites harmless without rendering a score or analysis.
    """
    return None
...
- When forecast backtest evidence is available, use the numeric validation errors to explain historical forecast performance and limitations.
...
def render_visual_ai_action(evidence, key, label="Explain chart"):
...
def render_chart(data, chart, key):
    ...
    if figure is not None:
        ...
        if st.session_state.get("show_advanced_management_tools", False):
...
def customize_dashboard_plan(plan, data, profile, key_prefix="dashboard"):
    ...
    with st.expander("Edit charts", expanded=False):
        st.caption(
    ...
    if actual_budget and actual_budget.get("variance_pct") is not None:
        ...
        if quality_intel.get("score") is not None:
            ...
            variance_text = (
                ...
                else "Data quality checks are available in the Data quality workspace."
            )
        else:
            missing = quality.get("missing_rate_pct")
            variance_text = (
...
def build_contextual_ai_questions(
    ...
    if forecast_result and forecast_result.get("forecasts"):
        ...
        if limited:
            ...
            add(
                ...
                "and selected model, and explain the validation limitations.",
                "forecast",
                94,
                f"{metric} currently has limited forecast validation support.",
            )
        else:
            add(
    ...
    if quality_status in {"Watch", "Limited"}:
        add(
            ...
            f"Data-quality status is currently {quality_status}.",
        )
...
def backtest_quarter_forecast(history, metric, model_id="linear"):
    ...
    if working.empty or len(working) < 2:
        return {
            ...
            "smape_pct": None,
            "confidence": "Limited",
            "confidence_reason": "At least two historical quarters are needed for walk-forward validation.",
        }
    ...
    if backtest_points < 2:
        ...
        confidence_reason = (
            ...
            "Validation support is limited by the selected history containing at most four quarters."
        )
        ...
        confidence_reason = (
            ...
            "Validation support is limited because the sample is short."
        )
    else:
        confidence = "Limited"
        confidence_reason = (
...
def _comparison_name_tokens(value):
    """Normalize a header into semantic tokens for schema-flexible matching."""
    text = re.sub(r"[^a-z0-9]+", " ", str(value).strip().lower())
    stop = {"the", "a", "an", "of", "for", "to", "total", "amount", "value", "metric", "measure"}
    return {token for token in text.split() if token and token not in stop}


def _numeric_signature(frame, column):
    values = pd.to_numeric(frame[column], errors="coerce").dropna()
    if values.empty:
        return {"count": 0, "mean_abs": None, "non_null_ratio": 0.0}
    return {
        "count": int(values.count()),
        "mean_abs": float(values.abs().mean()),
        "non_null_ratio": float(values.count() / max(len(frame), 1)),
    }


def _category_signature(frame, column):
...
def _column_pair_score(first_bundle, second_bundle, first_col, second_col, kind, first_index, second_index, first_count, second_count):
    """Score whether two differently named columns represent the same comparable field.

    The score intentionally combines header semantics with data shape/signature so
    quarter comparisons do not depend on identical headers. It never matches a
    numeric field to a categorical field.
    """
    first_name = str(first_col).strip().lower()
    second_name = str(second_col).strip().lower()
    first_tokens = _comparison_name_tokens(first_col)
    second_tokens = _comparison_name_tokens(second_col)
    union = first_tokens | second_tokens
    token_score = (len(first_tokens & second_tokens) / len(union)) if union else 0.0
    text_score = SequenceMatcher(None, first_name, second_name).ratio()

    max_positions = max(first_count, second_count, 1)
    position_score = 1.0 - min(abs(first_index - second_index) / max_positions, 1.0)

    if kind == "numeric":
        a = _numeric_signature(first_bundle["data"], first_col)
        b = _numeric_signature(second_bundle["data"], second_col)
        if not a["count"] or not b["count"]:
            return -1.0
        density_score = 1.0 - min(abs(a["non_null_ratio"] - b["non_null_ratio"]), 1.0)
        av = a["mean_abs"] or 0.0
        bv = b["mean_abs"] or 0.0
        if av == 0 and bv == 0:
            scale_score = 1.0
        elif av == 0 or bv == 0:
            scale_score = 0.0
        else:
            ratio = max(av, bv) / max(min(av, bv), 1e-12)
            scale_score = max(0.0, 1.0 - min(abs(__import__('math').log10(ratio)) / 3.0, 1.0))
        return 45 * token_score + 20 * text_score + 15 * density_score + 10 * scale_score + 10 * position_score

    a = _category_signature(first_bundle["data"], first_col)
    b = _category_signature(second_bundle["data"], second_col)
    if not a["count"] or not b["count"]:
        return -1.0
    density_score = 1.0 - min(abs(a["non_null_ratio"] - b["non_null_ratio"]), 1.0)
    max_unique = max(a["unique"], b["unique"], 1)
    cardinality_score = 1.0 - min(abs(a["unique"] - b["unique"]) / max_unique, 1.0)
    return 50 * token_score + 20 * text_score + 15 * cardinality_score + 10 * density_score + 5 * position_score


def comparable_columns(first_bundle, second_bundle, kind="numeric"):
    """Pair comparable quarter columns even when headers changed between periods.

    Matching order:
    1. exact normalized header;
    2. detected business concepts (revenue, cost, profit, etc.);
    3. deterministic schema/data-signature scoring for remaining same-type fields.
    """
    first_cols = list(first_bundle.get("profile", {}).get(kind, []))
    second_cols = list(second_bundle.get("profile", {}).get(kind, []))
    pairs = common_columns(first_cols, second_cols)
    used_first = {a for a, _ in pairs}
    used_second = {b for _, b in pairs}

    if kind == "numeric":
        concept_keys = [
            "actual", "revenue", "budget", "forecast", "costs", "profit", "ebitda",
            "opex", "capex", "headcount", "volume", "price", "prior_year", "margin",
        ]
    else:
        concept_keys = ["dimension"]

    for key in concept_keys:
        a = first_bundle.get("business", {}).get(key)
        b = second_bundle.get("business", {}).get(key)
        if a in first_cols and b in second_cols and a not in used_first and b not in used_second:
            pairs.append((a, b))
            used_first.add(a)
            used_second.add(b)

    remaining_first = [c for c in first_cols if c not in used_first]
    remaining_second = [c for c in second_cols if c not in used_second]
    scored = []
    for i, a in enumerate(remaining_first):
        for j, b in enumerate(remaining_second):
            score = _column_pair_score(
                first_bundle, second_bundle, a, b, kind,
                i, j, len(remaining_first), len(remaining_second),
            )
            scored.append((score, i, j, a, b))

    # Prefer the strongest one-to-one matches. A modest threshold allows renamed
    # but structurally equivalent columns through while rejecting weak cross-pairs.
    threshold = 24.0 if kind == "numeric" else 28.0
    for score, _, _, a, b in sorted(scored, reverse=True, key=lambda item: item[0]):
    ...
    # Structural fallback: when both quarters expose the same number of still-
    # unmatched fields of the same detected type, preserve their source order.
    # This handles generic headers such as Col1/Value A versus Measure X/Current.
    left_first = [c for c in first_cols if c not in used_first]
    left_second = [c for c in second_cols if c not in used_second]
    if left_first and len(left_first) == len(left_second):
        for a, b in zip(left_first, left_second):
            pairs.append((a, b))
...
def compare_quarter_bundles(
    ...
    common_numeric = comparable_columns(
        first_bundle,
        second_bundle,
        kind="numeric",
    )

    common_categories = comparable_columns(
        first_bundle,
        second_bundle,
        kind="categories",
    )
    ...
    if (
        ...
        and first_dimension in first_bundle["data"].columns
        and second_dimension in second_bundle["data"].columns
    ):
        # Both columns were independently detected as the primary business
        # dimension, so header equality is not required across quarters.
        category_first = first_dimension
        category_second = second_dimension
...
def build_pdf_export(quarter, source, summary, management_summary, data, plan, export_state=None):
    ...
    if not forecast_result or not forecast_result.get("forecasts"):
        ...
        for metric in forecast_result.get("metrics", []):
            ...
            backtest = (forecast_result.get("backtests", {}) or {}).get(metric, {}) or {}
            selection = (forecast_result.get("model_selection", {}) or {}).get(metric, {}) or {}
            rows = [["Measure", "Selected model", "Latest actual", "End forecast", "WAPE"]]
            rows.append([
                metric,
...
def build_powerpoint_export(quarter, source, summary, management_summary, data, plan, export_state=None):
    ...
    if forecast_result and forecast_result.get("forecasts"):
        ...
        for metric in forecast_result.get("metrics", []):
            ...
            actual = frame[frame["Series"] == "Actual"]
            projected = frame[frame["Series"] == "Forecast"]
            rows = [["Selected model", "Latest actual", "End forecast", "WAPE", "MAE"]]
            rows.append([
                selection.get("model_name", "Flat baseline"),
                f"{float(actual['Value'].iloc[-1]):,.2f}" if not actual.empty else "",
                f"{float(projected['Value'].iloc[-1]):,.2f}" if not projected.empty else "",
...
st.caption(
    "See what changed, what matters, and what to investigate next."
)
...
# ============================================================
# SMALL PRESENTATION PREVIEW
# ============================================================

def render_small_presentation_preview(data, plan, quarter, source, summary, key_prefix="mini_presentation"):
    """Render a compact slide-like dashboard while the normal page remains visible.

    This is intentionally a second view of the same deterministic dashboard data.
    It does not fetch Workiva again and does not call AI.  The same plan/figures are
    reused so the preview stays aligned with the charts shown on the main page.
    """
    if not plan:
        return
    ...
    st.caption(
        "Compact live view for an iframe, meeting share, or a small presentation window. "
        "Use `?embed=true` or `?presentation=true` on the deployed URL to open only this style of view."
    )

    # Centre the preview so it reads visually like a small 16:9 screen rather than
    # another full-width dashboard section.
    outer_left, preview_col, outer_right = st.columns([0.55, 4.9, 0.55])
    with preview_col:
        with st.container(border=True):
            st.markdown(f"**{plan.get('title', 'Dashboard')} · {quarter}**")
            st.caption(f"{source.get('spreadsheet_name')} → {source.get('sheet_name')}")

            kpis = _dashboard_kpi_rows(data, plan)[:4]
            if kpis:
                kpi_cols = st.columns(len(kpis))
                for idx, (label, value) in enumerate(kpis):
                    kpi_cols[idx].metric(str(label), _export_text(value))

            mini_charts = [c for c in plan.get("charts", []) if c.get("visible", True)][:2]
            if mini_charts:
                chart_cols = st.columns(2)
                for index, chart in enumerate(mini_charts):
                    try:
                        fig = build_chart_figure(data, chart)
                        if fig is None:
                            continue
                        fig.update_layout(
                            height=225,
                            margin=dict(l=12, r=12, t=38, b=18),
                        ...
                        chart_cols[index].caption(
                            f"{chart.get('title', 'Chart')}: {friendly_error(error)}"
                        )
            else:
                st.info("No chart is available for the presentation preview.")

        # A presentation URL opens a new Streamlit session, so session_state is not
        # shared with the dashboard tab. Carry the user's dashboard request in the
        # URL and reconstruct the same validated plan in presentation mode.
        active_request = str(st.session_state.get("agent_request", "") or "").strip()
        presentation_query = (
            f"?presentation=true&quarter={quote(str(quarter), safe='')}"
            f"&dashboard_request={quote(active_request, safe='')}"
        )
        st.link_button(
...
with st.sidebar:
    ...
    try:
        requested_quarter = st.query_params.get("quarter")
        if isinstance(requested_quarter, (list, tuple)):
            requested_quarter = requested_quarter[-1] if requested_quarter else None
    except Exception:
        requested_quarter = None

    quarter_index = 0
    if requested_quarter in quarter_options:
        quarter_index = quarter_options.index(requested_quarter)
    ...
    st.caption("Select the quarter you want to review.")
    ...
    with st.expander("View settings", expanded=False):
        display_mode = st.selectbox(
            ...
            index=0,
            help=(
                "Focus is the recommended default and shows only the most important information first. "
                "Standard shows the full dashboard. Accessible reading increases text size and spacing."
            ),
            key="display_mode",
        )
    ...
    with st.expander("Advanced", expanded=False):
        show_advanced_tools = st.checkbox(
            ...
            value=False,
            help=(
                "Adds the AI dashboard builder and chart-level AI explanations."
            ),
            key="show_advanced_management_tools",
        )

    show_presentation_preview = st.checkbox(
        "Show small presentation preview",
        value=True,
        key="show_presentation_preview",
        help=(
            "Shows a compact 16:9-style preview on the Overview page while keeping "
            "the full dashboard and graphs visible."
        ),
    )
...
presentation_mode = (
    PRESENTATION_QUERY_MODE
    or EMBED_QUERY_MODE
    or display_mode == "Presentation (PowerPoint)"
)
...
# ============================================================
# PRESENTATION / EMBED MODE (SMALL 16:9 WEB VIEW)
# ============================================================

if presentation_mode:
    # Compact presentation surface with the prompted dashboard plus the
    # supporting Financials, Trends, and Data quality tabs.
    st.markdown(
        """
        <style>
        ...
        [data-testid="stMainBlockContainer"] {
            ...
            padding: 0.55rem 0.8rem 0.45rem 0.8rem !important;
        }
        h1 { font-size: 1.25rem !important; margin: 0 !important; line-height: 1.1 !important; }
        h2 { font-size: 1.05rem !important; margin: 0.1rem 0 !important; line-height: 1.1 !important; }
        h3 { font-size: 0.9rem !important; margin: 0.05rem 0 !important; line-height: 1.1 !important; }
        p, li, label, [data-testid="stCaptionContainer"] { font-size: 0.72rem !important; line-height: 1.2 !important; }
        [data-testid="stMetric"] { padding: 0.28rem 0.45rem !important; }
        [data-testid="stMetricLabel"] { font-size: 0.65rem !important; }
        [data-testid="stMetricValue"] { font-size: 1.22rem !important; line-height: 1.05 !important; }
        [data-testid="stMetricDelta"] { font-size: 0.62rem !important; }
        [data-testid="stVerticalBlock"] { gap: 0.35rem !important; }
        [data-testid="stHorizontalBlock"] { gap: 0.5rem !important; }
        [data-testid="stPlotlyChart"] { margin: 0 !important; }
        [data-testid="stDataFrame"] { font-size: 0.66rem !important; }
        [data-testid="stTabs"] [role="tablist"] {
    ...
    # A presentation URL opens a new Streamlit session. Rebuild the exact
    # prompted dashboard from the request carried in the URL when necessary.
    presentation_plan = st.session_state.get("agent_plan")
    presentation_request = str(PRESENTATION_REQUEST_QUERY or "").strip()

    if presentation_plan is None and presentation_request:
        try:
            raw_presentation_plan = create_dashboard_plan(
                presentation_request,
                summary,
            )
            presentation_plan = validate_plan(
                raw_presentation_plan,
                profile,
            )
            st.session_state["agent_plan"] = presentation_plan
            st.session_state["agent_request"] = presentation_request
            st.session_state["agent_request_input"] = presentation_request
        except Exception as error:
            st.warning(
                "The prompted dashboard could not be reconstructed for presentation view. "
                f"Showing the executive fallback instead. {friendly_error(error)}"
            )

    if presentation_plan is None:
        presentation_plan = manual_plan("Executive", profile, business)

    st.markdown(f"## {presentation_plan.get('title', 'Dashboard')} · {selected_quarter}")
    if presentation_plan.get("reason"):
        st.caption(presentation_plan.get("reason"))
    st.caption(f"{source.get('spreadsheet_name')} → {source.get('sheet_name')}")

    dashboard_tab, financial_tab, trends_tab, quality_tab = st.tabs(
        ["Dashboard wall", "Financials", "Trends", "Data quality"]
    )

    with dashboard_tab:
        kpis = _dashboard_kpi_rows(data, presentation_plan)
        if kpis:
            kpi_columns = st.columns(min(4, len(kpis)))
        ...
        charts = [
            chart for chart in presentation_plan.get("charts", [])
            ...
            for index, chart in enumerate(charts):
                try:
                    ...
                    chart_columns[index % 2].plotly_chart(
                        fig,
                        use_container_width=True,
                        key=f"presentation_prompted_chart_{index}",
                    )
                except Exception as error:
                    chart_columns[index % 2].caption(
                        f"{chart.get('title', 'Chart')}: {friendly_error(error)}"
                    )
        else:
            st.info("No charts are available for the active dashboard request.")

    with financial_tab:
        financial = _financial_export_payload(data, business)
        metric_items = financial.get("metrics", [])[:6]
        if metric_items:
            cols = st.columns(min(3, len(metric_items)))
            for index, item in enumerate(metric_items):
                cols[index % len(cols)].metric(
                    str(item.get("concept", "Metric")).title(),
                    f"{float(item.get('total', 0)):,.0f}",
                )

        if financial.get("dimension_chart") is not None:
            fig = financial["dimension_chart"]
            fig.update_layout(
                height=390,
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key="presentation_financial_dimension",
            )
        else:
            derived = financial.get("derived_measures", {}) or {}
            if derived:
                st.dataframe(
                    pd.DataFrame([
                        {"Measure": name, "Formula": formula}
                        for name, formula in list(derived.items())[:10]
                    ]),
                    use_container_width=True,
                    hide_index=True,
                    height=300,
                ...
                st.info("No financial dimension chart was detected for this quarter.")

    with trends_tab:
        trend_figures = _trend_export_figures(data, profile)[:4]
        if trend_figures:
            trend_columns = st.columns(2)
            for index, (metric, fig) in enumerate(trend_figures):
                fig.update_layout(
                    height=235,
                    margin=dict(l=12, r=12, t=34, b=12),
                    title=dict(text=str(metric), font=dict(size=12)),
                    font=dict(size=9),
                )
                trend_columns[index % 2].plotly_chart(
                    fig,
                    use_container_width=True,
                    key=f"presentation_trend_{index}",
                )
        else:
            st.info("No time-series trend charts are available for this quarter.")

    with quality_tab:
        analytical_quality = (
            summary.get("analytical_context", {}).get("data_quality", {}) or {}
        )
        qcols = st.columns(4)
        qcols[0].metric("Rows", f"{profile.get('rows', 0):,}")
        qcols[1].metric("Columns", f"{profile.get('columns', 0):,}")
        qcols[2].metric(
            "Missing cells",
            f"{analytical_quality.get('missing_cells', 0):,}",
        )
        qcols[3].metric(
            "Missing rate",
            f"{analytical_quality.get('missing_rate_pct', 0):.1f}%",
        )
        missing = analytical_quality.get("most_missing_columns", []) or []
        if missing:
            st.dataframe(
                pd.DataFrame(missing).rename(columns={
                    "column": "Column",
                    "missing": "Missing",
                    "missing_pct": "Missing %",
                }),
...
# ============================================================
# MANAGEMENT PREFERENCES REMOVED
# ============================================================
summary = deepcopy(summary)
summary["management_watchlist"] = []
st.session_state["management_watchlist"] = []
...
primary_workspace = st.radio(
    ...
    help="Start with Overview. Use Explore for financials and trends.",
)

if primary_workspace == "Explore":
    explore_workspace = st.radio(
        "Explore",
        ["Financials", "Trends"],
        horizontal=True,
        key="explore_workspace",
        help="Choose the type of analysis you want to inspect.",
    )
    active_workspace = explore_workspace

elif primary_workspace == "More":
    more_workspace = st.selectbox(
        "More analysis",
        [
            "Forecasting",
            "Comparison",
        ...
        key="more_workspace",
        help="Additional analysis tools.",
    )
    active_workspace = more_workspace

else:
    active_workspace = primary_workspace
...
if active_workspace == "Overview":
    ...
    if board_mode == "Data quality":
        ...
        # ----------------------------------------------------
        # SMALL PRESENTATION PREVIEW
        # Keeps the normal page and graphs visible while also
        # showing a compact slide-like live view.
        # ----------------------------------------------------
        if st.session_state.get("show_presentation_preview", True) and not accessible_mode:
            render_small_presentation_preview(
                data,
                display_plan,
                selected_quarter,
                source,
                summary,
                key_prefix=f"mini_{selected_quarter}_{board_mode}",
            )
...
if active_workspace == "Forecasting":
    ...
    if forecast_result and forecast_result.get("forecasts"):
        ...
        for metric_index, metric in enumerate(forecast_result["metrics"]):
            ...
            card4.metric(
                "Backtest WAPE",
                f"{wape:.1f}%" if wape is not None else "N/A",
            )
