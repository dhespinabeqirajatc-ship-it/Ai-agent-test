...
if presentation_mode:
    # `?embed=true` now deliberately opens this compact graph-first surface.
    # A live Streamlit page is not part of the PPTX file itself. This mode is a
    # compact web surface intended for PowerPoint web-page add-ins (for example
    # LiveWeb), Teams/meeting screen share, or a browser window positioned over
    # a slide. It intentionally avoids the normal app chrome and long scrolling.
    st.markdown(
        """
        <style>
        /* Fit a dashboard into a 16:9 presentation viewport. */
    ...
    # Presentation mode mirrors the active prompted dashboard. A query/link view
    # creates a new Streamlit session, so rebuild the dashboard from the request
    # carried in the URL when session_state does not contain the original plan.
    ...
    if kpis:
        ...
        for idx, (label, value) in enumerate(kpis[:8]):
            kpi_columns[idx % len(kpi_columns)].metric(str(label), _export_text(value))
