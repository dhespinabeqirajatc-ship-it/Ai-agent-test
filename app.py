...
def _metric_business_importance(metric):
...
def build_materiality_ranking(summary, forecast_result=None, comparison=None, limit=8):
    ...
    def add(signal_type, title, detail, metric, magnitude, consistency, source):
        importance = _metric_business_importance(metric)
...
def build_contextual_ai_questions(summary, forecast_result=None, previous_quarter=None, selected_quarter=None, comparison_result=None):
    """Create high-value Ask AI shortcuts from the current calculated state."""
    pulse = build_management_pulse(summary, forecast_result)
    questions = [
        ("Explain top priority", pulse["trend_question"]),
        (
            "Prioritize watch items",
            "Explain the deterministic materiality ranking in priority order. Use the supplied "
            "scores and evidence only; do not invent a different ranking or accounting threshold.",
        ),
    ]
    ...
    if comparison_result and comparison_result.get("variance_bridge"):
        bridge = comparison_result.get("variance_bridge") or {}
        questions.insert(1, (
            "Explain variance bridge",
            f"Explain the current {bridge.get('first_quarter')} to {bridge.get('second_quarter')} variance bridge for "
            f"{bridge.get('metric')} by {bridge.get('dimension')}. Reconcile the starting total, category contributions, "
            "and ending total. Treat contribution as arithmetic, not causation.",
        ))
    elif pulse.get("forecast_question"):
...
# Lightweight deterministic analysis-readiness gate.
...
if quality_status in {"Strong", "Good"}:
    st.success(f"{quality_label} — {quality_guidance}")
elif quality_status == "Watch":
    st.warning(f"{quality_label} — {quality_guidance}")
else:
    st.error(f"{quality_label} — {quality_guidance}")
