...
def render_small_presentation_preview(data, plan, quarter, source, summary, key_prefix="mini_presentation"):
    ...
    with preview_col:
        with st.container(border=True):
            ...
            with header_left:
                st.markdown(f"**{plan.get('title', 'Executive dashboard')} · {quarter}**")
                st.caption(f"{source.get('spreadsheet_name')} → {source.get('sheet_name')}")
            with header_right:
                quality = summary.get("data_quality_intelligence", {}) or {}
                quality_text = quality.get("status", "Unavailable")
                quality_score = quality.get("score")
                if quality_score is not None:
                    quality_text += f" · {quality_score:.0f}/100"
                st.metric("Confidence", quality_text)
...
with st.sidebar:
    ...
    with st.expander("Advanced", expanded=False):
        show_advanced_tools = st.checkbox(
            ...
            value=False,
            help=(
                "Adds watchlists, detailed decision signals, the unified management brief, "
                "AI dashboard builder, and chart-level AI explanations."
            ),
...
if presentation_mode:
    ...
    # Reuse the normal deterministic dashboard plan and calculations.
    if st.session_state.get("agent_plan") is None:
        st.session_state["agent_plan"] = manual_plan("Executive", profile, business)
    presentation_plan = st.session_state.get("agent_plan") or manual_plan("Executive", profile, business)

    # Compact header: enough provenance for a presentation, without app chrome.
    header_left, header_right = st.columns([3.2, 1])
    with header_left:
        st.markdown(f"## {presentation_plan.get('title', 'Executive dashboard')} · {selected_quarter}")
        st.caption(f"{source.get('spreadsheet_name')} → {source.get('sheet_name')}")
    with header_right:
        quality = summary.get("data_quality_intelligence", {}) or {}
        quality_text = quality.get("status", "Unavailable")
        ...
        if quality_score is not None:
            quality_text += f" · {quality_score:.0f}/100"
        st.metric("Data confidence", quality_text)

    wall_tab, financial_tab, trends_tab, quality_tab, ai_tab = st.tabs(
        ["Dashboard wall", "Financials", "Trends", "Data quality", "AI summary + audit"]
    )

    with wall_tab:
        kpis = _dashboard_kpi_rows(data, presentation_plan)[:4]
        if kpis:
            kpi_columns = st.columns(len(kpis))
            for idx, (label, value) in enumerate(kpis):
                kpi_columns[idx].metric(str(label), _export_text(value))

        charts = presentation_plan.get("charts", [])[:4]
        if charts:
            ...
            for index, chart in enumerate(charts):
                try:
                    ...
                    chart_columns[index % 2].plotly_chart(
                        fig,
                        use_container_width=True,
                        key=f"presentation_wall_chart_{index}",
                    )
                except Exception as error:
            ...
            st.info("No dashboard charts are available for this quarter.")
    ...
    with financial_tab:
        ...
        if financial.get("dimension_chart") is not None:
            ...
            fig.update_layout(height=390, margin=dict(l=12, r=12, t=36, b=12), font=dict(size=9))
            st.plotly_chart(fig, use_container_width=True, key="presentation_financial_dimension")
        else:
            derived = financial.get("derived_measures", {}) or {}
            if derived:
                st.dataframe(
                    pd.DataFrame(
                        [{"Measure": name, "Formula": formula} for name, formula in list(derived.items())[:10]]
                    ),
                    use_container_width=True,
                    hide_index=True,
                    height=300,
    ...
    with quality_tab:
        quality = summary.get("data_quality_intelligence", {}) or {}
        analytical_quality = summary.get("analytical_context", {}).get("data_quality", {}) or {}
        qcols = st.columns(4)
        qcols[0].metric("Rows", f"{profile.get('rows', 0):,}")
        qcols[1].metric("Columns", f"{profile.get('columns', 0):,}")
        qcols[2].metric("Missing cells", f"{analytical_quality.get('missing_cells', 0):,}")
        qcols[3].metric("Missing rate", f"{analytical_quality.get('missing_rate_pct', 0):.1f}%")
        ...
        guidance = quality.get("analysis_guidance")
        if guidance:
            st.caption(guidance)

    with ai_tab:
        # Presentation-only executive narrative and governance evidence. This
        # keeps the slide-friendly surface self-contained without exposing the
        # full desktop workspace.
        # Reuse the same management summary already generated in the main
        # dashboard. Presentation mode is only another view of the same
        # quarter, so it should not invalidate or hide an existing summary.
        # The button below can still explicitly regenerate it when requested.

        summary_col, audit_col = st.columns([1.55, 1])

        with summary_col:
            st.markdown("### AI management summary")
            st.caption("Generate a grounded executive summary for this quarter and dashboard plan.")

            generate_col, status_col = st.columns([1, 1.15])
            with generate_col:
                generate_clicked = st.button(
                    "Generate / refresh summary",
                    key="presentation_generate_management_summary_button",
                    use_container_width=True,
                )
            with status_col:
                if st.session_state.get("management_summary"):
                    st.caption("Summary is ready for this presentation view.")
                else:
                    st.caption("No presentation summary generated yet.")
            ...
            if generate_clicked:
                try:
                    ...
                    log_event("Generate presentation AI summary", selected_quarter)
                except Exception as error:
                    st.session_state["management_summary"] = (
                        "AI summary unavailable: " + friendly_error(error)
                    )
                    st.warning(st.session_state["management_summary"])

            if st.session_state.get("management_summary"):
                st.markdown(st.session_state["management_summary"])
                render_ai_evidence_indicator(
                    summary,
                    purpose="presentation management summary",
                    key="presentation_management_summary_evidence",
                )

        with audit_col:
            st.markdown("### Audit log")
            st.caption("Latest session activity. Workiva write actions remain disabled.")
            audit_rows = st.session_state.get("audit_log", []) or []
            if audit_rows:
                st.dataframe(
                    pd.DataFrame(audit_rows).tail(12),
                    use_container_width=True,
                    hide_index=True,
                    height=360,
                )
