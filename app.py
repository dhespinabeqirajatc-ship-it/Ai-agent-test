...
- Describe direction, magnitude, peaks/lows, anomalies, and relationships only when directly supported.
...
def chat_answer(
    ...
    trends_context = {
        ...
        "correlations": compact_ai_context(summary).get("correlations", []),
    }
...
- For trend questions, use the Trends board context and describe direction, magnitude, peaks/lows, anomaly screening signals, and correlations only when supported.
...
"""
    ...
    evidence = {
        ...
        "numeric_statistics": summary.get("numeric_stats", {}),
        "category_information": summary.get("categories", {}),
        "analytical_context": compact_ai_context(summary),
...
def build_management_pulse(summary, forecast_result=None):
    ...
    if forecast_result:
        ...
        quarters = forecast_result.get("historical_quarters", []) or []
...
def build_forecast_result(bundles, selected_metrics, horizon):
...
def generate_forecast_ai_commentary(forecast_result):
    ...
    for metric, frame in forecast_result.get("forecasts", {}).items():
        payload["metrics"][metric] = frame[["Quarter", "Value", "Series"]].to_dict(orient="records")
...
- If history is too short or unstable, explicitly say forecast confidence is limited.
