...
def render_small_presentation_preview(data, plan, quarter, source, summary, key_prefix="mini_presentation"):
    ...
    with preview_col:
        ...
        # Query-only URL keeps the link relative to the deployed Streamlit app.
        presentation_query = f"?presentation=true&quarter={str(quarter).replace(' ', '%20')}"
        st.link_button(
...
if presentation_mode:
    ...
    # Presentation mode now mirrors the active prompted dashboard rather than
    # switching to a separate set of standard dashboards. If the user has not
    # prompted a dashboard yet, the deterministic executive plan remains the fallback.
